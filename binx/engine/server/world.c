...
// get the clipping hull
	if ((ent->v->solid == SOLID_BSP || ent->v->solid == SOLID_BSPTRIGGER || ent->v->solid == SOLID_PORTAL) && mdlidx)
	{
		...
		if (model && (model->type == mod_alias)) {
			// This is where the problem was at, we can't do proper model tracing if we set it to NULL in case of a mod_alias.
		} else if (!model || (model->type != mod_brush && model->type != mod_heightmap))
		{
